...
class QuotesListView: UIViewController, QuotesListViewProtocol {
    ...
    var presenter: QuoteListPresenter & QuotesListOutputInteractorProtocol = QuoteListPresenter()
    var quoteList = [Quote]()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        addQuotesTableView()
        presenter.createQuotesListModule2(quotesView: self)
...
extension QuotesListView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        presenter.showQuoteSelection(with: quoteList[indexPath.row], from: self)
    }
